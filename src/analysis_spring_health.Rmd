...
Assuming depression at baselines means not none or N/A at baseline and removing those indiviudals
```{r}
...
```{r}
avg_work_productivity = data_spring_health_clean %>%
...
Recreate data sets to allow for non-parameteric tests
```{r}
...
According to my analysis, both PHQ-9 and unproductive SDS days average percentage change are statistically signifantly greater than zero and both percentage changes are positive indicating there is evidence the Spring Health platform is not effective at reducing thes two outcomes. 
