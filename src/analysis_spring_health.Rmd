...
```{r}
average_change_phq9_fun = function(data = "data_spring_health_clean")  {
...
average_change_phq9 = average_change_phq9_fun()
average_change_phq9
```
Step: Q5 What is the average change in total PHQ9 score for depressed individuals using the platform

Assuming depression at baselines means not none or N/A at baseline and removing those individuals.
```{r}
average_change_phq9_depressed_fun = function(data = "data_spring_health_clean") {
  out_data =  data_spring_health_clean %>%
  filter(questionnaire_kind == "PHQ9") %>%
...
average_change_phq9_depressed = average_change_phq9_depressed_fun()
...
avg_work_productivity_fun = function(data = "data_spring_health_clean") {
  ...
  arrange(desc(id), (event)) %>%
    ...
    # Only keep values with at least two responses
    filter(n() > 1) %>%
    mutate(sds_days_score_change = tsibble::difference(sds_days_unproductive) / (ifelse(lag(sds_days_unproductive) == 0, 1, lag(sds_days_unproductive)))) %>%
    ungroup() %>%
    summarise(sds_days_score_change = mean(sds_days_score_change, na.rm = TRUE)) %>%
    mutate(sds_days_score_change = formattable::percent(sds_days_score_change, digits = 2))
  return(data_out)
}
...
```
Step: Conduct non-parameteric wilcox tests to evaluate statistical differenes
```{r}
...
According to my analysis, both PHQ-9 and unproductive SDS days average percentage change are statistically significantly greater than zero and both percentage changes are positive indicating there is evidence the Spring Health platform is not effective at reducing these two outcomes. 
